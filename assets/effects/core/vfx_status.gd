@tool
extends VFXInstance
class_name VFXStatusEffect
## Specialized status effect aura controller for continuous elemental & status auras
## (fire, lightning, healing, shatter). Exposes unified color, noise, and shape uniforms.
...
## The tertiary color of this effect.
...
## Controls the curve of the transition from primary_color to secondary_color
...
## Lifetime of the flame/aura particles.
...
## Explosiveness of the particles.
...
@export var emission_shape : Vector3 = Vector3(0.5, 1.0, 0.5):
	set(v):
		emission_shape = v
		for p in particles:
			if p.process_material and "emission_shape_scale" in p.process_material:
...
@export var particles_orbit : float = 0.3:
	set(v):
		particles_orbit = v
		for p in particles:
			if p.process_material and "orbit_velocity" in p.process_material:
...
@export var particles_radial : float  = 0.2:
	set(v):
		particles_radial = v
		for p in particles:
			if p.process_material and "radial_accel" in p.process_material:
...
@export var particles_vertical : float = 0.4:
	set(v):
		...
		for p in particles:
			if p.process_material and "gravity" in p.process_material:
